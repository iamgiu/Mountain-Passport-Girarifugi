...
class RifugioRepository(private val context: Context) {
    ...
    suspend fun isRifugioSaved(userId: String, rifugioId: Int): Boolean {
        return try {
            ...
            doc.toObject(UserRifugioInteraction::class.java)?.isSaved ?: false
        } catch (e: Exception) {
            false
        }
    }
    ...
    suspend fun toggleSaveRifugio(userId: String, rifugioId: Int, isSaved: Boolean) {
        try {
            ...
            updateRifugioStats(rifugioId, isSaved)
        } catch (e: Exception) {
            throw e
        }
    }
    ...
    suspend fun addTestReviews(rifugioId: Int) {
        ...
        val testReviews = listOf(
            Review(
                ...
                rating = 4.5f,
                comment = "Rifugio fantastico con vista mozzafiato! Il personale è stato molto gentile e il cibo ottimo.",
            ...
            Review(
                ...
                rating = 5.0f,
                comment = "Esperienza indimenticabile! Il rifugio è pulito e confortevole. Tornerò sicuramente.",
                timestamp = com.google.firebase.Timestamp.now()
            ),
            Review(
                rifugioId = rifugioId,
