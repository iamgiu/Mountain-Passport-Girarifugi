...
class GroupsLeaderboardAdapter : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    private var groups: List<LeaderboardUser> = emptyList()

    fun submitList(newGroups: List<LeaderboardUser>) {
        groups = newGroups
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        when (holder) {
            ...
            is RowViewHolder -> {
                ...
                if (userIndex < groups.size) {
                    val user = groups[userIndex]
                    holder.bind(user)
                }
            }
        }
